...
const Timetable = (locationtheaters) => {
  ...
  const movietimelist = locationTheatersArray?.filter(
    (element) =>
      ...
      <div class="flex flex-wrap w-96 mr-4 ml-28">
        {selectmovie &&
          ...
          movietimelist?.map((item, index) => (
            <div
              class="mx-2 w-24 m-4 border border-light-grey-500 hover:border-transparent rounded"
              key={index}
              onClick={() =>
                selecttimehandler(item.start_time, item.cinema_type)
              }
            >
              <div
                style={{
                  backgroundColor:
                    item.start_time === selecttime &&
                    item.cinema_type === selectcinematype
                      ? "#D3D3D3"
                      : "transparent",
                  cursor: "pointer",
                  textAlign: "center",
