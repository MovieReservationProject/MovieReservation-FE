...
const Calendar = (locationtheaters , myreserveNum, mytitleKorean ,mycinemaName ,mymoviedate) => {
   ...
   const reserveNum = locationtheaters.myreserveNum
    ...
    const getDatesStartToLast = (startDate, lastDate) => {
      ...
      const movenextweek=()=>{
          const startDate2= new Date(dayjs(new Date(reserveweeklist[0])).add(7,"day").format("YYYY-MM-DD"))
          const lastDate2= new Date(dayjs(startDate2).add(7,"day").format("YYYY-MM-DD"))
        getDatesStartToLast(startDate2,lastDate2)

      }

      const moveprevweek=()=>{
        const startDate2= new Date(dayjs(new Date(reserveweeklist[0])).format("YYYY-MM-DD"))
        const lastDate2= new Date(dayjs(startDate2).add(-7,"day").format("YYYY-MM-DD"))
        ...
        <>
             <div style={{ position: 'relative', top: '-20px', left : '30px' , margin: '8px' }}>
               {(reserveweeklist && reserveweeklist.length>0) && result[index-1]==='n' && <div>{reserveweeklist[index+1]?.substring(6,7)}</div>}
                ...
                key={index} onClick={()=>setReservetdate(date)}>
                  <div style={{backgroundColor : date === selectdate ? '#D3D3D3' :'transparent' , cursor: 'pointer'}} class="hover:border-transparent rounded-full">
                     {date.substring(8,10)}
                     </div>
                     <div>
