...
const Mychangereserve = (locationtheaters , myreserveNum, mytitleKorean ,mycinemaName ,mymoviedate,myreserveid) => {
    ...
    const handlechangeClick = (reserveId) => {
      ...
      const changedata = (reserveId) => {
        ...
        const encodedReserveNumwithouthypen = encodeURIComponent(reserveNum.replace('-', ''));
    ...
    return (
        ...
        <div >
                   <button class="" onClick={()=>handlechangeClick(reserveId)}>예약변경</button>
                   </div>
        </div>
    );
};
